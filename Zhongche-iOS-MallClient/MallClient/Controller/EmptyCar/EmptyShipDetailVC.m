...
@interface EmptyShipDetailVC ()<UIAlertViewDelegate>
...
@property (nonatomic, strong) UIView       *viShipMessage;
@property (nonatomic, strong) UILabel      *lbShipNum;
...
@implementation EmptyShipDetailVC
...
- (void)bindView {
    ...
    self.viShipMessage.frame = CGRectMake(0, self.viHead.bottom + 10, SCREEN_W, 200);
    [self viShipMessageMake];
    [self.svGoodsDetail addSubview:self.viShipMessage];


    self.viSeller.frame = CGRectMake(0, self.viShipMessage.bottom + 10, SCREEN_W, 120);
    [self viSellerMake];
...
- (void)viShipMessageMake {

    UILabel *lbTitle = [self labelWithText:@"班轮信息" WithFont:[UIFont systemFontOfSize:14] WithTextAlignment:NSTextAlignmentLeft WithTextColor:[UIColor blackColor]];
    lbTitle.frame = CGRectMake(15, 15, SCREEN_W - 15, 20);
    [self.viShipMessage addSubview:lbTitle];
    ...
    [self.viShipMessage addSubview:lbLine];

    self.lbShipNum.frame = CGRectMake(15, lbLine.bottom + 15, SCREEN_W - 15, 10);
    [self.viShipMessage addSubview:self.lbShipNum];

    self.lbCarryStation.frame = CGRectMake(15, self.lbShipNum.bottom + 10, SCREEN_W - 15, 10);
    [self.viShipMessage addSubview:self.lbCarryStation];

    self.lbTakeStation.frame = CGRectMake(15, self.lbCarryStation .bottom + 10, SCREEN_W - 15, 10);
    [self.viShipMessage addSubview:self.lbTakeStation];

    self.lbStartTime.frame = CGRectMake(15, self.lbTakeStation .bottom + 10, SCREEN_W - 15, 10);
    [self.viShipMessage addSubview:self.lbStartTime];

    self.lbEndTime.frame = CGRectMake(15, self.lbStartTime .bottom + 10, SCREEN_W - 15, 10);
    [self.viShipMessage addSubview:self.lbEndTime];

    self.lbLeaveTime.frame = CGRectMake(15, self.lbEndTime .bottom + 10, SCREEN_W - 15, 10);
    [self.viShipMessage addSubview:self.lbLeaveTime];
...
- (void)bindModel {
    ...
    if (self.current) {
        ...
        self.lbShipNum.text = [NSString stringWithFormat:@"航次：%@",self.current.shipNum];
        self.lbShipNum.attributedText = [self attributedStrWithString:self.lbShipNum.text];

        self.lbCarryStation.text = [NSString stringWithFormat:@"装货港：%@",self.current.shipStartStation];
        self.lbCarryStation.attributedText = [self attributedStrWithString:self.lbCarryStation.text];

        self.lbTakeStation.text = [NSString stringWithFormat:@"卸货港：%@",self.current.shipEndStation];
        self.lbTakeStation.attributedText = [self attributedStrWithString:self.lbTakeStation.text];

        self.lbStartTime.text = [NSString stringWithFormat:@"开仓时间：%@",[self stDateToString:self.current.shipStartTime]];
        self.lbStartTime.attributedText = [self attributedStrWithString:self.lbStartTime.text];

        self.lbEndTime.text = [NSString stringWithFormat:@"截载时间：%@",[self stDateToString:self.current.shipEndTime]];
        self.lbEndTime.attributedText = [self attributedStrWithString:self.lbEndTime.text];

        self.lbLeaveTime.text = [NSString stringWithFormat:@"离港时间：%@",[self stDateToString:self.current.shipLeaveTime]];
        self.lbLeaveTime.attributedText = [self attributedStrWithString:self.lbLeaveTime.text];
...
- (UIView *)viShipMessage {
    if (!_viShipMessage) {
        _viShipMessage = [UIView new];
        _viShipMessage.backgroundColor = [UIColor whiteColor];

    }
    return _viShipMessage;
}
