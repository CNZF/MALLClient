...
-(void)setImages:(NSArray *)images
{
    ...
    if (_images.count > 0) {
        ...
        if (_images.count <= 1) {
            self.collectionVi.scrollEnabled = NO;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([obj isKindOfClass:[NSString class]]) {
        [cell.imgVi sd_setImageWithURL:[NSURL URLWithString:obj] placeholderImage:[UIImage imageNamed:@"占位图.png"] options:SDWebImageRefreshCached completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
            if (image) {
                [self.imageArr replaceObjectAtIndex:indexPath.row withObject:image];

            }
            else {
                [self.imageArr replaceObjectAtIndex:indexPath.row withObject:[UIImage imageNamed:@"占位图.png"]];
...
-(void)updateCollectionViewContentOffset
{
    ...
    point.x = (int)(self.collectionVi.contentOffset.x / self.frame.size.width) * self.frame.size.width + self.frame.size.width;
    point.y = self.collectionVi.contentOffset.y;
