...
- (instancetype)init
{
    self = [super init];
    if (self) {
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(isFourDays:) name:@"isFourDays" object:nil];
        
    }
    return self;
}

- (void)binView:(NSString *)isfour {
    ...
    [self viewMake:isfour];
}


- (void)isFourDays:(NSNotification *)obj
{
    NSString * isFour = obj.object;
    [self binView:isFour];
}

- (void) viewMake:(NSString *)isfour  {
    ...
    sv.frame = CGRectMake(0, viTitle.bottom, SCREEN_W, SCREEN_H - viTitle.bottom- kiPhoneFooterHeight);
    ...
    UIView *viMoth1= [self mothViewWith:startday1 isFour:isfour];
    ...
    UIView *viMoth2= [self mothViewWith:startday2 isFour:isfour];
    ...
    UIView *viMoth3= [self mothViewWith:startday3 isFour:isfour];
    viMoth3.origin = CGPointMake(0, viMoth2.bottom);
    [sv addSubview:viMoth3];

     sv.contentSize = CGSizeMake( SCREEN_W, viMoth3.bottom +240);
...
- (UIView *)mothViewWith:(NSDate *)startDate isFour:(NSString *)isfour{
    ...
    for (int i = 0; i<endDayInfo.day; i++) {
        ...
        if ([isfour isEqualToString:@"NO"]) {
            if (dayInfo.day < currentDayInfo.day && dayInfo.month == currentDayInfo.month) {
                [btn setTitleColor:[UIColor grayColor] forState:UIControlStateNormal];
                btn.enabled = NO;
            }
        }else{
            NSInteger bewdays = [self getChaDays:[NSString stringWithFormat:@"%i-%i-%i",currentDayInfo.year,currentDayInfo.month,currentDayInfo.day] curDay:[NSString stringWithFormat:@"%i-%i-%i",dayInfo.year,dayInfo.month,dayInfo.day]];
