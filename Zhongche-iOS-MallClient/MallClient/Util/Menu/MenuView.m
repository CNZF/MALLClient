...
-(void)show{
    ...
    [UIView animateWithDuration:0.5 animations:^{
        ...
        ws.leftMenuView.frame = self.menuViewframe;
        ws.coverView.frame    = self.coverViewframe;
        ws.coverView.alpha = CoverViewAlpha;
...
-(void)openMenuView{
    ...
    [UIView animateWithDuration:0.5 animations:^{
        CGFloat x           = 0;
        CGFloat y           = ws.menuViewframe.origin.y;
        CGFloat w           = ws.menuViewframe.size.width;
        CGFloat h           = ws.menuViewframe.size.height;
        ws.leftMenuView.frame = CGRectMake(x, y, w, h);
        
        ws.coverView.frame    = ws.coverViewframe;
        ws.coverView.alpha    = CoverViewAlpha;
    }];
}
...
-(void)closeMenuView{
    ...
    [UIView animateWithDuration:0.5 animations:^{
        ...
        CGFloat x           = -ws.menuViewframe.size.width;
        CGFloat y           = ws.menuViewframe.origin.y;
        CGFloat w           = ws.menuViewframe.size.width;
        CGFloat h           = ws.menuViewframe.size.height;
        ws.leftMenuView.frame = CGRectMake(x, y, w, h);//self.LeftViewFrame;
        ws.coverView.frame    = CGRectMake(0, 0,kSCREEN_WIDTH, ws.menuViewframe.size.height);
...
-(void)coverTap{
    ...
    [UIView animateKeyframesWithDuration:0.5 delay:0 options:UIViewKeyframeAnimationOptionLayoutSubviews animations:^{
        ws.leftMenuView.frame = CGRectMake(-ws.menuViewframe.size.width, 0, ws.menuViewframe.size.width, ws.menuViewframe.size.height);
        ws.coverView.frame    = CGRectMake(0, 0,kSCREEN_WIDTH, kSCREEN_HEIGHT);
        ws.coverView.alpha    = 0.0;
    } completion:^(BOOL finished) {
        [ws.coverView removeFromSuperview];
        [ws.leftMenuView removeFromSuperview];
    }];
