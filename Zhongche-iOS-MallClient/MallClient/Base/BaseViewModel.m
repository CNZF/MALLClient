...
- (instancetype)init {
    ...
    if ([BaseViewModel checkNetworkStatus] == NotReachable) {
        [[Toast shareToast]makeText:@"网络不可用，请检查网络" aDuration:1];
        return nil;
    }
    self = [super init];
    if (self) {
...
- (void)initHeadDic {
    ...
    [self.dicHead setValue:@"b8965bb83a326abe0895441c51fa996d" forKey:@"clientid"];
    ...
    if (info) {
        ...
        [self.dicHead setValue:info.organization_id forKey:@"companyId"];
    }
...
+(NetworkStatus)checkNetworkStatus {
    NetworkStatus networkType;
        id status = [Reachability reachabilityForInternetConnection];
