...
{
    CapacityEntryModel *cpy  = [[CapacityEntryModel allocWithZone:zone] init];
    //    cpy.nickname = self.nickname;
    cpy.startPlace = self.startPlace;
    cpy.endPlace = self.endPlace;
    cpy.shipmentsTime = self.shipmentsTime;
    cpy.km = self.km;
    return cpy;
}
...
{
    return @{@"price":@[@"ticketPrice",@"price",@"price20t"],@"lineTypeChoose":@"lineType",@"ID":@"id"};
}
...
{
    if ([lineType isEqualToString:@"LINE_TYPE_CONTAINER_20"]||[lineType isEqualToString:@"BUSINESS_TYPE_CONTAINER"]) {
        _lineType = @"20英尺通用箱";
    }
...
- (TransportationModel *)transportationModel{
    if (!_transportationModel) {
        _transportationModel = [[TransportationModel alloc] init];
    }
    return _transportationModel;
}
...
- (void)setCapacityType:(NSString *)capacityType{
    ...
    NSDictionary *dic= @{@"集装箱运力":@"BUSINESS_TYPE_CONTAINER",@"散堆装运力":@"BUSINESS_TYPE_BULK_STACK",@"三农化肥运力":@"BUSINESS_TYPE_CHEMICAL",@"批量成件运力":@"BUSINESS_TYPE_BATCH",@"冷链运力":@"BUSINESS_TYPE_COLD_CHAIN",@"大件运力":@"BUSINESS_TYPE_LARGE_SIZE",@"商品车运力":@"BUSINESS_TYPE_VECHICLE",@"液态运力":@"BUSINESS_TYPE_LIQUID",@"一带一路运力":@"BUSINESS_TYPE_ONE_ROAD",@"快速配送运力":@"BUSINESS_TYPE_MULTI_TRANSPORT"};
...
- (void)setLineTypeChoose:(NSString *)lineTypeChoose{
    ...
    _lineTypeChoose = lineTypeChoose;

    NSDictionary *dic= @{@"BUSINESS_TYPE_CONTAINER":@"集装箱运力",@"BUSINESS_TYPE_BULK_STACK":@"散堆装运力",@"BUSINESS_TYPE_CHEMICAL":@"三农化肥运力",@"BUSINESS_TYPE_BATCH":@"批量成件运力",@"BUSINESS_TYPE_COLD_CHAIN":@"冷链运力",@"BUSINESS_TYPE_LARGE_SIZE":@"大件运力",@"BUSINESS_TYPE_VECHICLE":@"商品车运力",@"BUSINESS_TYPE_LIQUID":@"液态运力",@"BUSINESS_TYPE_ONE_ROAD":@"一带一路运力",@"BUSINESS_TYPE_MULTI_TRANSPORT":@"快速配送运力"};

    self.businessTypeCode = [dic valueForKey:lineTypeChoose];
